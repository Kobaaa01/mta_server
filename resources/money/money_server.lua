...
function loadPlayerMoney(player, nickname)
    local query = dbQuery(db, "SELECT money_pocket FROM Users WHERE nickname = ?", nickname)
    local result = dbPoll(query, -1)

    if result and result[1] then
        local money = result[1].money_pocket
        triggerClientEvent(player, "onReceiveMoney", player, money)
        setElementData(player, "money_pocket", money)
    else
